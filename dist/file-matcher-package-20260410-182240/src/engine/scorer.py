...
def get_vendor_similarity(ledger_vendor: str, file_vendor_tokens: list[str]) -> float:
    ...
    Used by veto logic. Uses token_set_ratio (not partial_ratio): the latter
    lets unrelated short strings (e.g. 'ring') score high against long ledger
    names; token_set still treats 'vyo' vs 'vyo structural' as a strong match.
    """
    if not file_vendor_tokens:
        return 0.0
