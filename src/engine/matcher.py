...
import config
from src.engine.scorer import score_pair, _amounts_within_tolerance
...
def run_matching(
    ...
    for row in ledger_rows:
        ...
        if row.index in row_to_match:
            ...
            status = _classify(match.score, candidates, row.index, row_to_match)
    ...
    for (vendor_key, ca_key), rows in groups.items():
        ...
        for r1, r2 in combinations(rows, 2):
            ...
            for fi, fr in enumerate(file_records):
                ...
                file_vendor_str = " ".join(fr.vendor_tokens).lower()
                has_vendor = (vendor_key in file_vendor_str or file_vendor_str in vendor_key) if fr.vendor_tokens else True

                if has_amount and has_ca and has_vendor:
                    # Create sum-match candidates for both rows
                    for row in (r1, r2):
                        sc = ScoredCandidate(
                            file=fr,
                            score=config.AMOUNT_SUM_WEIGHT + (config.CA_MATCH_WEIGHT if ca_key in fr.ca_codes else 0) + (config.VENDOR_STRONG_WEIGHT if has_vendor else 0),
...
def _classify(
    ...
    """Classify match status based on score and candidate pool."""
    if score >= config.STRONG_THRESHOLD:
        return "Strong"

    if score >= config.GOOD_THRESHOLD:
        match = row_to_match.get(row_idx)
        if match:
            near_ties = [
                c for c in candidates
                if not c.vetoed
                and c.score > 0
                and c.file.original_name != match.file.original_name
                and abs(c.score - score) <= 10
            ]
            if near_ties:
                return "Review"
        return "Good"
